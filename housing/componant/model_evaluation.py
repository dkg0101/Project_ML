...
class ModelEvaluation:
    ...
    def get_best_model(self):
        try:
            model =None
            model_evaluation_file_path = self.model_evaluation_config.model_evaluaion_file_path

            # For running pipeline first time , there will be no file available, then create file and return model as none
            if not os.path.exists(model_evaluation_file_path):
                write_yaml_file(file_path=model_evaluation_file_path,
                                )
                return model
            #if file already exists , get content in dictionary formate
            model_eval_file_content = read_yaml_file(file_path=model_evaluation_file_path)

            #if file is empty, initiate empty dictionary o.w. read content 
